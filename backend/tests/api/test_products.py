...
def test_update_product(client: TestClient):
    ...
    update_data = {"title": "Updated Product Title", "price": 39.99}
...
def test_create_product_validation_errors(
    client: TestClient, session: Session, invalid_product: dict
):
...
def test_product_category_relationship(client: TestClient, session: Session):
    ...
    update_response = client.put(
        f"/products/{product.id}", json={"category_id": category2.id}
    )
    assert update_response.status_code == 200
...
def test_product_with_missing_image_returns_null_url(
